...
const Usefetch2 = (domain, entrance = false, method = 'POST', body = null) => {
  ...
  const doFetch = useCallback(async (_body = null) => {
   ...
   //   url: `${baseUrl}${url}${domain}`,
    ...
    await fetch(`${baseUrl}${urluser}${domain}`, method == 'POST' ? config_with_post : config_with_get)
      .then((response) => {
